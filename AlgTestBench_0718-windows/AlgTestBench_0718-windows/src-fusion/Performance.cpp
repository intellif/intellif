...
void NameListItem::addMatchFace(boost::shared_ptr<FaceRecoder>& rFaceRcd, float fScore)
{
	...
	{
		//vector<pair<unsigned int, boost::shared_ptr<FaceRecoder>>> tVector;
		//SortMapByValue(mNewNameList, tVector);
...
Performance::Performance(string strPathCfg)
{
	...
	//detect init
	mdwDetectType = CascadeDetector::FACE_CASCADE_DET_TYPE_INVALID;
}
...
Performance::~Performance()
{
	...
	***/
	...
	/***
	for (int dwHandleIdx = 0; dwHandleIdx < mdwExtractHandleCnt; dwHandleIdx++)
	{
		if_handle_t hExtractHandle = mExtractHandleFifo->GetIFaceHandler();

		if (hExtractHandle == NULL)
		{
			std::cout << "Can not destroy detect handle: " << std::endl;
			continue;
		}
...
int Performance::LoadConfigInformation(void)
{
	...
	if (mdwAlgVersions == ALG_EXTRA_TYPE_DEEPEYE || mdwAlgVersions == ALG_EXTRA_TYPE_TUAN)
	{
		IfFeatureExtManager::IfFeatureExtMangerInit();
		mdwFaceFeatureLen = 128;
	}
	else if (mdwAlgVersions == ALG_EXTRA_TYPE_ORG)
	{
		mdwFaceFeatureLen = 181;
	}
	else if (mdwAlgVersions == ALG_EXTRA_TYPE_FUSION)
	{
		FusionFeatureExtManager::IfFeatureExtMangerInit();
		mdwFaceFeatureLen = 256;
	}
	...
	if (mdwOldDetect)
	{
		g_vecIntDetect.push_back(CascadeDetector::FACE_CASCADE_DET_TYPE_ORG);
		mdwDetectType = CascadeDetector::FACE_CASCADE_DET_TYPE_ORG;
	}
	if (mdwNickleDetect)
	{
		g_vecIntDetect.push_back(CascadeDetector::FACE_CASCADE_DET_TYPE_NICKLE);
	}
	if (mdwVjboostDetect)
	{
		g_vecIntDetect.push_back(CascadeDetector::FACE_CASCADE_DET_TYPE_VJBOOST);
		mdwDetectType = CascadeDetector::FACE_CASCADE_DET_TYPE_VJBOOST;
	}
	if (mdwMtcnnDetect)
	{
		g_vecIntDetect.push_back(CascadeDetector::FACE_CASCADE_DET_TYPE_MTCNN);
		mdwDetectType = CascadeDetector::FACE_CASCADE_DET_TYPE_MTCNN;
...
#pragma region detecthandle
	/*
    for(int dwHandleIdx = 0; dwHandleIdx < mdwDetectHandleCnt; dwHandleIdx++)
    {
		if_handle_t newDetectHandle = CascadeDetectManager::CreateIfDetectorHandle(0);

        if(newDetectHandle == 0)
        {
            std::cout << "LoadConfigInformation, Can not create detect handle: " << dwHandleIdx << std::endl;
            continue;
        }

        mDetectHandleFifo->ReturnIFaceHandler(newDetectHandle);
    }
	*/
#pragma endregion
#pragma region extracthandle
	/*
    for(int dwHandleIdx = 0; dwHandleIdx < mdwExtractHandleCnt; dwHandleIdx++)
    {
		if_handle_t newExtractHandle = IfMutilAlgExtManger::CreateIfExtratorHandle(mStrModelPath);

        if(newExtractHandle == 0)
        {
            std::cout << "LoadConfigInformation, Can not create extract handle: " << dwHandleIdx << std::endl;
            continue;
        }

        mExtractHandleFifo->ReturnIFaceHandler(newExtractHandle);
    }
	*/
#pragma endregion
...
//检测人脸特征值
int Performance::DetectAndExtractFace(if_handle_t hDetectHandle, if_handle_t hExtractHandle, const std::string &picturePath, float * & pfFeatureWrap)
{
	bool bLocalUrl = true;
...
#pragma region facedetect
	...
	//if_handle_t    hDetectHandle = mDetectHandleFifo->GetIFaceHandler();0
...
int Performance::DoFaceDetectLoop(int dwThreadIdx)
{
	madwDetectStatus[dwThreadIdx] = 0;
...
#pragma region facedetect
		printf("CascadeDetectManager::FeedOneFramToDetector## start::path=%s\n", picturePath.c_str());
...
#pragma endregion
		...
		printf("Detect result::tOneFaceRect,path=%s,left=%d,right=%d,top=%d,bottom=%d\n", picturePath.c_str(), tOneFaceRect.Rect.left, tOneFaceRect.Rect.right, tOneFaceRect.Rect.top, tOneFaceRect.Rect.bottom);
...
int Performance::DoFaceExtractLoop(int dwThreadIdx)
{
	madwExtractStatus[dwThreadIdx] = 0;
