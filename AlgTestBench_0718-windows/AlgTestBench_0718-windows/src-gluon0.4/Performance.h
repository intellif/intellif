...
public:
	...
	int  DoFaceDetectLoop(int dwThreadIdx);
	int  DoFaceExtractLoop(int dwThreadIdx);
...
private:
    ...
    static const int MAX_FEATURE_PADDED_LEN  = 256;
	...
	int    DetectAndExtractFace(if_handle_t hDetectHandle, if_handle_t hExtractHandle, const std::string &picturePath, float *  & pfFeatureWrap);
