...
#ifdef _MSC_VER
	string strModelPath = g_strAlgModelPath;
#else
	string strModelPath = g_strAlgModelPath;
#endif
...
FaceExtractor::FaceExtractor()
{
	...
	else if (g_mdwAlgVersion == ALG_EXTRA_TYPE_PANDA)
	{
		pFaceAlgExtraType[dwAlgNums] = ALG_EXTRA_TYPE_PANDA;
		pFaceAlgExtraHandl[dwAlgNums] = YtFeatureExtManager::CreateIfExtratorHandle(1);
		dwAlgNums += 0x1;
	}
}
...
FaceExtractor::~FaceExtractor()
{
    ...
    for (int dwAlgIdx = 0; dwAlgIdx < dwAlgNums; dwAlgIdx++)
    {
		...
		else if (pFaceAlgExtraType[dwAlgIdx] == ALG_EXTRA_TYPE_PANDA)
		{
			cout << "FaceExtractor::~FaceExtractor=ALG_EXTRA_TYPE_PANDA=ALG_EXTRA_TYPE_PANDA#####" << std::endl;
			YtFeatureExtManager::DestoryIfExtratorHandle(pFaceAlgExtraHandl[dwAlgIdx]);
			pFaceAlgExtraHandl[dwAlgIdx] = INVALID_HANDL;
		}
    }
        ...
        {             
			...
			else if (dwAlgType == ALG_EXTRA_TYPE_GLUON)
			{
				...
				if (g_flipExtratFeature)
				{
					...
					pNomalFeatureLen = GluonFeatureExtManager::ExtratFeatureFromImage(pFaceAlgExtraHandl[dwAlgIdx], rMatOrg, pFaceRect, &pNomalFeature);

					if ((pNomalFeatureLen != g_mdwAlgFeatureLen) || (pNomalFeature == NULL))
					{
						if (pNomalFeature != NULL)
						{
							GluonFeatureExtManager::ReleaseFeautreResult(pNomalFeature);
							pNomalFeature = NULL;
						}
					...
					if_rect_t  pFaceRectFlip(pFaceRect);
					...
					pFlipFeatureLen = GluonFeatureExtManager::ExtratFeatureFromImage(pFaceAlgExtraHandl[dwAlgIdx], matFlip, pFaceRectFlip, &pFlipFeature);

					if ((pFlipFeatureLen != g_mdwAlgFeatureLen) || (pFlipFeature == NULL))
					{
						if (pFlipFeature != NULL)
						{
							GluonFeatureExtManager::ReleaseFeautreResult(pFlipFeature);
							pFlipFeature = NULL;
						}
			...
			else if (dwAlgType == ALG_EXTRA_TYPE_PANDA)
			{
				dwRetLen = YtFeatureExtManager::ExtratFeatureFromImage(pFaceAlgExtraHandl[dwAlgIdx],
					rMatOrg.data,
					rMatOrg.cols,
					rMatOrg.rows,
					pFaceRect,
				...
				if ((dwRetLen != g_mdwAlgFeatureLen) || (pRetFeatureTmp == NULL))
				{
					...
					if (pRetFeatureTmp != NULL)
					{
						YtFeatureExtManager::ReleaseFeautreResult(pRetFeatureTmp);
						pRetFeatureTmp = NULL;
					}
					dwRetLen = 0;
				}
...
void IfMutilAlgExtManger::ReleaseFeautreResult(float *pFeature, int dwAlgType)
{
	...
	{
		YtFeatureExtManager::ReleaseFeautreResult(pFeature);
	}
}
                                        ...
                                        int              dwTargType)
{
	...
	{
		PandaFeatureCompare(pFeature0, dwLen0, pFeature1, dwLen1, score);
	}
...
int IfMutilAlgCompare::PandaFeatureCompare(const float *   pfFeature0,
	int       dwFeatureLen0,
	const float *   pfFeature1,
	int       dwFeatureLen1,
	float *   pfScore)
{
	...
	if (dotFeature0 == 0 || dotFeature1 == 0)
	{
		*pfScore = (float)0.0;
		...
		std::cout << "compare, len0: " << dwFeatureLen0 << ", len1: " << dwFeatureLen1 << ", result: 0" << std::endl;
	...
	*pfScore = (float)fResult;
	...
	//std::cout << "compare, len0: " << dwFeatureLen0 << ", len1: " << dwFeatureLen1 << ", result: " << fResult << std::endl;
