...
namespace sdktest{
	...
	public:
		...
		void DrawPoint(Image *image, int x, int y, int stroke_width){
			...
			for(int h = x - stroke_width; h <= x + stroke_width; h++) {
				for(int v = y - stroke_width; v <= y + stroke_width; v++) {
					int dx = h - x, dy = v - y;
					if (!(h >= 0 && h < image->cols && v >= 0 && v < image->rows))
						continue;
		...
		void DrawRect(Image *image, int top, int left, int right, int bottom, int stroke_width){
			if(image->cols < stroke_width || image->rows < stroke_width)
				return;
			int ss, ci;
			if(image->type == Image::Image_Gray) {
				ss = 1;
			...
			if(top < 0) top = 0;
			if(top >= image->rows) top = image->rows-1;
			if(bottom < 0) bottom = 0;
			if(bottom >= image->rows) bottom = image->rows-1;

			if(left < 0) left = 0;
			if(left >= image->cols) left = image->cols - 1;
			if(right < 0) right = 0;
			if(right >= image->cols) right = image->cols - 1;

			for(int i=top;i<=bottom;i++){
				for(int sw = 0; sw < stroke_width; sw++) {
					int u = (left+sw)*ss+ci;
					int d = (right-sw)*ss+ci;
					image->data[i*image->stride+u]=255;
					image->data[i*image->stride+d]=255;
				}
			}
